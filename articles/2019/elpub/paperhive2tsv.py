...
lsID = sys.argv[1]
lsidrequest = requests.get('https://paperhive.org/api/document-items/by-document/external?type=langsci&id=' + lsID)
...
for i, discussion in enumerate(paperhiveformated['discussions']):  
    ...
    out.write("\t".join([str(bookID), str(commentID), str(proofreaderID), str(pagenumber), str(startpos), str(endpos), title.replace('"',"'"), '"%s"'%body.replace('"',"'").replace('\n',' ')]))
    out.write("\n") 
